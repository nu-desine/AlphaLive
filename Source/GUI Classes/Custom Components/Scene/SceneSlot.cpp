...
void SceneSlot::paint (Graphics &g)
{
    ...
    if (status == 0) //empty
    {
        ColourGradient seqGradient(Colours::black, (CIRCE_WIDTH*0.5), 0, Colours::darkgrey.withAlpha(0.5f), (CIRCE_WIDTH),(getHeight()), true);
        g.setGradientFill(seqGradient);
    }
    else if (status == 1) //settings applied but not selected
    {
        ColourGradient seqGradient(Colours::white.withAlpha(0.5f), (CIRCE_WIDTH*0.5), 0, Colours::grey.withAlpha(0.5f), (CIRCE_WIDTH*0.5),(getHeight()), true);
        g.setGradientFill(seqGradient);
    }
    else if (status == 2) //selected
    {
        ColourGradient seqGradient(AlphaTheme::getInstance()->colour2, (CIRCE_WIDTH*0.5), 0, AlphaTheme::getInstance()->colour1, (CIRCE_WIDTH*0.5),(getHeight()), true);
        g.setGradientFill(seqGradient);
    }

...
void GuiHighPassFilter::mouseEnter (const MouseEvent &e)
{
    ...
    {
        mainComponentRef.setInfoTextBoxText(translate("High-Pass Filter Mix Control. Sets the wet/dry ratio for High-Pass filter for the selected pads."));
        parameterHoverLabel->setText(String(mixSlider->getValue(), 3), false);
    }
    else if (frequencySlider->isMouseOver(true))
    {
        mainComponentRef.setInfoTextBoxText(translate("High-Pass Filter Cut-Off Frequency Control. Sets the cut-off frequency for High-Pass filter for the selected pads."));
        parameterHoverLabel->setText(String(frequencySlider->getValue(), 0), false);
    }
    else if (bandwidthSlider->isMouseOver(true))
    {
        mainComponentRef.setInfoTextBoxText(translate("High-Pass Filter Bandwidth Control. Sets the bandwidth for High-Pass filter for the selected pads."));

...

{
	...
	for (int i = 0; i <= 119; i++)
	{
		...
		if(noteIndex == 2 || noteIndex == 4 || noteIndex == 7 || noteIndex == 9 || noteIndex == 11)
		{
			keys.insert(i, new GuiKeys(keyNumber, Colours::grey, Colours::lightgrey, AlphaTheme::getInstance()->mainColourLighter));
...
void GuiPiano::setKeyDisplay (int keyNumber, bool keyOn)
{
    if (keyOn == false) //off
    {
        ...
        if(noteIndex == 2 || noteIndex == 4 || noteIndex == 7 || noteIndex == 9 || noteIndex == 11)
        {
            keys[keyNumber]->setColours(Colours::grey, Colours::lightgrey, AlphaTheme::getInstance()->mainColourLighter);
        }
        else if (noteIndex == 1)
        {
